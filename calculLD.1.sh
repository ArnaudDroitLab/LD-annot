...
awk '{if ($7 > "'$5'" && $1 != "0") sum+= ($5-$2)} END {print sum/NR}' mydata.ld > meanLD

awk '{if ($7 > "'$5'" && $1 != "0" && NR>"1") arr[NR]=($5-$2)} END { for (i in arr) print arr[i] }' mydata.ld \
| sort -n | awk '{arr[NR]=$1} END {print arr[(NR+1)/2]}' > medianLD
